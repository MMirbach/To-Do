...
def handle_message(update, context):
    ...
    update.message.reply_text(
        "Oh man, looks like our connection was lost "
        + "\U0001F62D"
...
def get_username(update, context):
    ...
    res = requests.get(
        "http://132.69.8.12:443/api/checkUsername", params=encodeDict(params)
    )
    if res.text == "true":
        context.user_data["username"] = username
        update.message.reply_text(
            f"Hi {username}, nice to see you! What's your password?"
        )
...
def get_password(update, context):
    ...
    if res.text == "true":
        ...
        update.message.reply_text(
            "Hey! don't try to trick me! that's not the password"
            + "\U0001F621"
...
def unsupported(update, context):
    update.message.reply_text(
        "FYI, this can't be added to your to-do list, "
        "but you can still send it to me " + "\U0001F604"
    )


def badCredentials(update, context):
    update.message.reply_text(
        "This has nothing to do with connecting to your account "
        + "\U0001F621"
